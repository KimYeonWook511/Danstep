...
const drawGreen = (keypoints: Keypoint[], canvas: OffscreenCanvas) => {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;

  const color = 'rgba(0,255,0,0.5)';
  const neonColor = 'rgba(0, 255, 0, 1)';
...
const drawRed = (keypoints: Keypoint[], canvas: OffscreenCanvas) => {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
...
self.onmessage = async (event) => {
    const { keypoints, canvas, color } = event.data;
    if (color === 'green') {
      drawGreen(keypoints, canvas);
    } else if (color === 'red') {
      drawRed(keypoints, canvas);
    }
